...
void Adafruit_Thermal::reset() {
  Clear();
  writeBytes(ASCII_ESC, '@'); // Init command, a794 compatible
  ...
  writeBytes(ASCII_ESC, 'D'); // Set tab stops..., a794 compatible
...
void Adafruit_Thermal::justify(char value){	//a794 compatible
  uint8_t pos = 0;
...
void Adafruit_Thermal::feed(uint8_t x) {	//a794 compatible
...
void Adafruit_Thermal::feedRows(uint8_t rows) {	//a794 compatible
...
void Adafruit_Thermal::flush() {	//a794 compatible
  writeBytes(ASCII_FF);
}
...
void Adafruit_Thermal::setSize(char value){
  ...
  writeBytes(ASCII_GS, '!', size);		//A794 compatible
...
// 1 - normal underline
// 2 - thick underline
void Adafruit_Thermal::underlineOn(uint8_t weight) {		//A794 compatible
  if(weight > 2) weight = 2;
  writeBytes(ASCII_ESC, '-', weight);
}

void Adafruit_Thermal::underlineOff() {						//A794 compatible
  writeBytes(ASCII_ESC, '-', 0);
}

void Adafruit_Thermal::printFullBitmap(const uint8_t *bitmap, bool fromProgMem)
{
  writeBytes(ASCII_GS, '*');
  writeBytes(72, 20);
  for(int i = 0; i < 72*20*8; i++)
  {
	stream->write(fromProgMem ? pgm_read_byte(bitmap++) : *(bitmap++));
  }
}

void Adafruit_Thermal::printToPaper()
{
	writeBytes(0x1D, 0x2F, 0x0);
}

void Adafruit_Thermal::printFullBitmap(const uint8_t *bitmap, int width, int height, bool fromProgMem)
{
  writeBytes(ASCII_GS, '*');
  writeBytes(width/8, height/8);
  width /= 8;
...
void Adafruit_Thermal::printBitmap(
 int w, int h, const uint8_t *bitmap, bool fromProgMem) {
  ...
  if(h > 64) h = 64;
  width = (w + 7) / 8;	//8 bits per row byte
  height = (h+7) / 8;	//8 bytes per column
  writeBytes(ASCII_GS, '*');
  writeBytes(width, height);
  for(x=0; x < width * 8; x++) {	//iterate through all columns
    for(y = 0; y < height; y++)
	{
        stream->write(fromProgMem ? pgm_read_byte(bitmap++) : *(bitmap++));
	}
...
/*
  ...
  for(i=rowStart=0; rowStart < h; rowStart += chunkHeightLimit) {
    ...
    writeBytes(ASCII_GS, '*');
	writeBytes(chunkHeight, rowBytesClipped);
...
void Adafruit_Thermal::setLineHeight(int val) { //A794 compatible
  if(val < 24) val = 24;
  lineSpacing = val - 24;
...
void Adafruit_Thermal::Knive() {
#ifdef A794
  //writeBytes(26);	//perform knive cut
  writeBytes(ASCII_ESC, 'm');	//perform knive cut
#endif
}
